<x-dashboard.layout>
    ...
    <x-slot name="toolbar">
        <x-dashboard.toolbar.main>
            ...
            <x-slot name="backbutton">
                <!--begin::Button-->
                <a href="{{ route('funding.index') }}" class="btn btn-sm btn-light-success fw-bolder">
    ...
    <x-slot name="post">
        <!--begin::form-->
        <form class="form" action="{{ route('funding.update', $funding->id) }}" method="POST" autocomplete="off"
                ...
                <div class="card-body">
                    <div class="mb-6">
                        ...
                        <div class="input-group">
                            <select class="form-select form-select-sm form-select-solid" name="period"
                                ...
                                <option value="{{ $funding->setting_id }}">
                                    {{ $funding->setting->period }}
                                </option>
                            </select>
                        </div>
                    </div>
                    <div class="mb-6">
                        ...
                        <div class="input-group input-group-sm">
                            <input class="form-control" type="text" name="title"
                                value="{{ old('title', $funding->title ?? '') }}" required>
                        </div>
                    </div>
                    <div class="mb-6">
                        <label class="form-label fs-7 required">Tanggal Kegiatan</label>
                        <div class="input-group input-group-sm">
                            <input class="form-control flatpickr" type="text" name="date_activity"
                                value="{{ old('date_activity', $funding->date_activity ?? '') }}" required>
                        </div>
                    </div>
                    <div class="mb-6">
                        <label class="form-label fs-7 required">Keterangan</label>
                        <div class="input-group input-group-sm">
                            <input class="form-control" type="text" name="description"
                                value="{{ old('description', $funding->description ?? '') }}" required>
                        </div>
                    </div>
                    <div class="mb-3">
                        ...
                        @if ($funding->attachment_pdf != null)
                            <div class="input-group input-group-sm pb-2">
                                <a href="{{ asset('storage/funding/' . $funding->attachment_pdf) }}"
                    ...
                    <div class="mb-3">
                        ...
                        @if ($funding->attachment_img != null)
                            <div class="input-group input-group-sm pb-2">
                                <a href="{{ asset('storage/funding/' . $funding->attachment_img) }}"
    ...
    <x-slot name="javascript">
        {{-- Start::Flatpickr --}}
        <script src="{{ asset('js/custom/flatpickr/id.js') }}"></script>
        <script>
            $(".flatpickr").flatpickr({
                altInput: true,
                locale: "id",
